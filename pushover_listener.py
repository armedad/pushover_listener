...
from .const import PUSHOVER_EVENT, DOMAIN
...
class PushoverClient:
    ...
    def __init__(
        ...
        self.secret = None
        self.device_id_pushover = None
        self._running = False
        self._ws_task = None
        self._reconnect_delay = INITIAL_RECONNECT_DELAY
        
        self.config_entry_id = config_entry_id
        self._ha_device_id_cache: str | None = None

        self._sensor_update_callback: Callable[[dict[str, Any]], None] | None = None
    ...
    @callback
    def set_sensor_callback(
        self, callback: Callable[[dict[str, Any]], None] | None
    ) -> None:
        """Set the callback for the sensor entity."""
        self._sensor_update_callback = callback

    @property
    def ha_device_id(self) -> str | None:
        """Get the cached Home Assistant device ID."""
        if self._ha_device_id_cache:
            return self._ha_device_id_cache
        ...
        if device:
            self._ha_device_id_cache = device.id
            return self.ha_device_id
        return None
    ...
    async def start(self) -> None:
        ...
        self.device_id_pushover = await self.load_cached_device_id()
        if not self.device_id_pushover:
            await self.register_device()
            await self.save_device_id(self.device_id_pushover)
    ...
    async def register_device(self) -> None:
        ...
        try:
            async with session.post(
                ...
                self.device_id_pushover = data["id"]
                _LOGGER.info("Registered device: %s", self.device_id_pushover)
    ...
    async def download_and_discard_old_messages(self) -> None:
        ...
        try:
            async with session.get(
                MESSAGES_URL,
                params={"secret": self.secret, "device_id": self.device_id_pushover},
            ) as resp:
                resp.raise_for_status()
    ...
    async def delete_messages_up_to(self, max_id: int) -> None:
        ...
        try:
            async with session.post(
                DELETE_URL_TEMPLATE.format(device_id=self.device_id_pushover),
    ...
    async def listen(self) -> None:
        ...
        while self._running:
            # --- START OF AIOHTTP FIX ---
            # 1. Get the shared session without calling __aenter__
            session = async_get_clientsession(self.hass)
            try:
                # 2. The 'async with' here manages the websocket, not the session
                async with session.ws_connect(WEBSOCKET_URL) as ws:
                    login_str = f"login:{self.device_id_pushover}:{self.secret}\n"
                    ...
                    async for msg in ws:
                        if msg.type == WSMsgType.BINARY:
                            # --- FIX: Handle keep-alive silently ---
                            if msg.data == b"#":
                                continue  # Keep-alive
            ...
            # 3. The 'finally' block that closed the session is GONE.
            # --- END OF AIOHTTP FIX ---
        ...
        try:
            async with session.get(
                ...
                for msg in messages:
                    ...
                    if self._sensor_update_callback:
                        self._sensor_update_callback(enriched)
