...
CONFIG_SCHEMA = vol.Schema(
    {
        DOMAIN: vol.All(cv.ensure_list, [USER_SCHEMA]),
    },
...
async def async_setup(hass: HomeAssistant, config: dict[str, Any]) -> bool:
    ...
    for conf in config[DOMAIN]:
        ...
        hass.data[f"{DOMAIN}_{device_name}"] = client

        try:
            await asyncio.wait_for(client.start(), timeout=30)
        except TimeoutError:
            logging.getLogger(__name__).error(f"Pushover client setup for {device_name} timed out")
            return False

        asyncio.create_task(client.listen())
        logging.getLogger(__name__).info(f"Pushover Listener for {device_name} started (async)")
